...
class Cost_calculator:
    ...
    def calculate_cost(self, method): 
        ...
        if method == 'qdrift' or method == 'rand_ham':
            ...
            if method == 'qdrift':
                self.costs['q_drift'] = methods_trotter.calc_qdrift_resources(lambd, N, deltaE = 1e-4, P_failure = .1)
            elif method == 'rand_ham':
                self.costs['rand_ham'] = methods_trotter.calc_rand_ham_resources(Lambd, lambd, Gamma, N, deltaE = 1e-4, P_failure = .1)
            ...
            if method == 'taylor_naive':
                self.costs['taylor_naive'] = methods_taylor.taylor_naive(Lambd, Gamma, N, epsilon_PEA, epsilon_HS, epsilon_S)
            ...
            if method == 'low_depth_trotter':
                ...
                self.costs['low_depth_taylor'] = methods_plane_waves.low_depth_taylor(N, lambd, Lambd, eps_PEA, eps_HS, eps_S, Ham_norm)
            elif method == 'low_depth_taylor_on_the_fly':
                self.costs['low_depth_taylor_on_the_fly'] = methods_plane_waves.low_depth_taylor_on_the_fly(N, eta, lambd, Omega, eps_PEA, eps_HS, eps_S, eps_tay, Ham_norm, J, x_max)
            ...
            if method == 'linear_t':
                self.costs['linear_t'] = methods_qrom.linear_T(N, lambd, eps_PEA, eps_S, Ham_norm)
            elif method == 'sparsity_low_rank':
                self.costs['sparsity_low_rank'] = methods_qrom.sparsity_low_rank(N, lambd, eps_PEA, eps_S, L, Ham_norm)
            ...
            if method == 'interaction_picture':
                self.costs['interaction_picture'] = methods_interaction_picture.interaction_picture(N, Gamma, lambd_T, lambd_U_V, eps_S, eps_HS, eps_PEA)
            elif method == 'sublinear_scaling':
                self.costs['sublinear_scaling'] = methods_interaction_picture.sublinear_scaling_interaction(N, eta, Gamma, lambd_T, lambd_U_V, eps_S, eps_HS, eps_PEA, eps_mu, eps_M_0, J)